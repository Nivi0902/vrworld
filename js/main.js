(function ($) {
  ...
  $(document).on("click", ".submit", function (event) {
    ...
    if (!error && actionURL) {
        ...
        // Trim the name, email, and phone fields before sending
        const nameField = formObj.find('input[name="name"]');
        const emailField = formObj.find('input[name="email"]');
        const phoneField = formObj.find('input[name="phone"]');

        if (nameField.length) nameField.val(nameField.val().trim());
        if (emailField.length) emailField.val(emailField.val().trim());
        if (phoneField.length) phoneField.val(phoneField.val().trim());
